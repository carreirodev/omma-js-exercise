...
btnEnviar.onclick = (evento) => {
	...
	let inputIngredientes = document.querySelector("#ingrediente");
...
function exibirReceitas() {
	...
	for (let index = 0; index < listaDeReceitas.length; index++) {
		htmlReceitas += `<article class="card">
					...
					<p>Preparo: ${listaDeReceitas[index].preparo}</p>        
				</article>`;
...
function deletarReceita(id) {
	const novaListaDeReceitas = listaDeReceitas.filter((receita) => receita.id != id);
...
const atualizaReceita = (id, receitaAtualizada) => {
	...
	console.log(foiAtualizado ? "Receita atualizada com sucesso!" : "Não foi encontrado o id");
};

const buscarReceita = (termoBuscado) => {
	const resultados = listaDeReceitas.filter(
		(receita) => receita.titulo.toLowerCase().indexOf(termoBuscado.toLowerCase()) != -1
	);
